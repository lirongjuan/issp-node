var app = angular.module('plan', [{
    files:[
        "root/personal/plan/_res/js/service.js"
    ]
}]);
app.controller('planCtrl',function ($scope,$state) {
    ...
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.personal.plan.edit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.aduit = function(){
        if($scope.getId){
            $state.go('root.personal.plan.aduit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.delete = function(){
        if($scope.getId){
            $state.go('root.personal.plan.list[12]',{id:$scope.getId,name:"delete",page:$scope.page});
