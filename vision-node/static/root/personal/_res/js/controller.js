var app = angular.module('business', [
    {files:[
    "root/personal/_res/js/service.js"
]}
]);
app.controller('businessCtrl', function ($scope,$state) {
    if ($state.current.url == '/personal') {//默认加载列表
        $state.go('root.personal.plan');
    }
    //父 Ctrl 监听到事件，向下广播
...
}).controller('navCtrl',function($scope,$state,$location,personalSer){
    $scope.navCla='move';
    var active =$location.path().split('/')[3];
    $scope.navCla=active?active:'move';
    $scope.navClass= function(name){
       $scope.navCla=name
    };
    personalSer.navPermission().then(function(response){   //权限设置
    ...
    personalSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"个人愿景管理",menuList:[{name1:"个人愿景计划",msg:'plan'},{name2:"职业规划定制",msg:'occupational'}], showIs: true},
