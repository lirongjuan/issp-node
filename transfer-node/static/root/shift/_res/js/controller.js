var app = angular.module('business', [
    {files:[
    "root/shift/_res/js/service.js"
]}
]);
app.controller('businessCtrl', function ($scope,$state) {
    if ($state.current.url == '/shift') {//默认加载列表
        $state.go('root.shift.move');
    }
    //父 Ctrl 监听到事件，向下广播
...
}).controller('navCtrl',function($scope,$state,$location,shiftSer){
    $scope.navCla='move';
    ...
    shiftSer.navPermission().then(function(response){   //权限设置
    ...
    shiftSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"物资调动",menuList:[{name1:"物资调动",msg:'move'}], showIs: true},
