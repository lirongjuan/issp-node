...
module.exports = function(){
    ...
    router.get('/contractquotedata/setButtonPermission', function*(){ //设置导航权限
        var $self = this;
    ...
    }).get('/contractquotedata/sonPermission', function*(){  //下拉导航权限
        var $self = this;
        var navToken = {token:$self.cookies.get('token')};
    ...
    }).get('/contractprojectinfo/guidePermission/:guideAddrStatus', function*(){ //基本信息菜单导航权限
        var $self = this;
    ...
    }).get('/contractquotedata/guidePermission/:guideAddrStatus', function*(){ //单价资料信息菜单导航权限
        var $self = this;
        var page = {name:$self.params.guideAddrStatus,token:$self.cookies.get('token')};
    ...
    }).get('/contractnodestandard/guidePermission/:guideAddrStatus', function*(){ //单价标准信息菜单导航权限
        var $self = this;
        var page = {name:$self.params.guideAddrStatus,token:$self.cookies.get('token')};
        yield (server().standardPermission(page)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/contractquotedata/uploadFile', koaBody({multipart:true}),function *(next) {//上传文件 合同单价资料管理
        var $self = this;
    ...
    }).get('/contractquotedata/listFile', function*(){ //查看附件 合同单价资料管理
        var $self = this;
        var enData = $self.request.query;
        enData.userToken = $self.cookies.get('token');
        yield (server().mateEnclosure(enData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/contractquotedata/downloadFile', function*(){//下载文件 合同单价资料管理
        var $self = this;
        var count = $self.request.query;
        var data = {
            path:count.path
        };
        yield (fetch(config()['rurl']+`/contractquotedata/v1/downloadFile${urlEncode(data,true)}`, {
            method : 'GET',
    ...
    }).post('/contractquotedata/delete', koaBody({multipart:true}), function*(){//合同单价资料管理 删除文件
        var $self = this;
        var delData = $self.request.body;
        delData.userToken = $self.cookies.get('token');
        yield (server().delmate(delData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/contractprojectinfo/templateExport', function*(){//导入模板下载 合同单价基本信息
        var $self = this;
        var fileName = '合同单价基本信息导入模板.xlsx';
        yield (fetch(config()['rurl']+`/contractprojectinfo/v1/templateExport`, {
            method : 'GET',
        }).then(function(res){
    ...
    }).post('/contractprojectinfo/importExcel', koaBody({multipart:true}),function *(next) {//导入 合同单价基本信息
        var $self = this;
    ...
    }).get('/contractprojectinfo/findApplyAreas', function*(){   //获取所有的内部项目地区名称
        var $self = this;
        var listData = $self.request.query;
        listData.userToken = $self.cookies.get('token');
        yield (server().ApplyAreas(listData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error)=>{
    ...
    }).get('/contractprojectinfo/allDispatchNum', function*(){  //获取所有派工单编号
        var $self = this;
    ...
    }).get('/contractprojectinfo/export', function*(){//导出 合同单价基本信息
        var $self = this;
        var count = $self.request.query;
        var fileName = "合同单价基本信息"+'.xlsx';
        yield (fetch(config()['rurl']+`/contractprojectinfo/v1/export${urlEncode(count,true)}`, {
            method : 'GET',
            headers : {'userToken' : $self.cookies.get('token')}
        }).then(function(res){
            $self.set('content-type', 'application/vnd.ms-excel;charset=utf-8');
    ...
    }).get('/contractprojectinfo/dispatchprojectinfo', function*(){ //根据派工单编号查询一个派工单信息
        var $self = this;
        var page = $self.request.query;
        page.userToken = $self.cookies.get('token');
        yield (server().getProjectByNum(page)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
