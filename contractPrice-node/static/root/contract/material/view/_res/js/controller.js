var app = angular.module('materivalView', ['toastr']);
app.controller('materivalViewCtrl', function($scope,materialSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    materialSer.viewMaterial(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.contract.material.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.contract.material.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/contractquotedata/downloadFile${encode(obj,true)}`;
