var app = angular.module('storeView', ['toastr']);
app.controller('storeViewCtrl', function($scope,storeSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    storeSer.viewStore(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.stock.store.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.stock.store.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/materialinstock/downloadFile${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloSigning.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/materialinstock/deleteFile',
                headers: {
