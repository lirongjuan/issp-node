...
app.controller('businessCtrl', function ($scope,$state) {
    ...
    materialSer.navPermission().then(function(response){   //权限设置
    ...
    materialSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"物资领用",menuList:[{name1:"物资领用归还",msg:'collar'},{name2:"项目组领用归还",msg:'restitution'}], showIs: true},
