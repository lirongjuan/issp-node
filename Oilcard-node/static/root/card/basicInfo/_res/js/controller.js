var app = angular.module('basicInfo', [{
    files:[
        "root/card/basicInfo/_res/js/service.js"
    ]
}]);
app.controller('basicInfoCtrl',function ($scope,$state) {
    ...
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.card.basicInfo.edit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.delete = function(){
        if($scope.getId){
            $state.go('root.card.basicInfo.list[12]',{id:$scope.getId,name:"delete",page:$scope.page});
    ...
    $scope.congeal = function(){
        if($scope.getId){
            $state.go('root.card.basicInfo.list[12]',{id:$scope.getId,name:'congeal',page:$scope.page})
