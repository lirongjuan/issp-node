var app = angular.module('rechargeView', ['toastr']);
app.controller('rechargeViewCtrl', function($scope,rechargeSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    rechargeSer.viewRecharge(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.card.recharge.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.card.recharge.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/recharge/download${encode(obj,true)}`;
