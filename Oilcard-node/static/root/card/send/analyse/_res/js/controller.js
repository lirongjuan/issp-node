var app = angular.module('sendAnaly', ['ng-pagination','toastr','angularjs-dropdown-multiselect']);
app.controller('sendAnalyCtrl',function($scope,sendSer,toastr){

    sendSer.GetNo().then(function (response){
    ...
    $scope.Anfn =function(){
        if($scope.selectAn){
            ...
            sendSer.sendAnalyse($scope.selectAn).then(function(response){
                if(response.data.code == 0){
