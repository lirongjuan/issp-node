...
app.controller('businessCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,cardSer){
    ...
    cardSer.navPermission().then(function(response){   //权限设置
    ...
    cardSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"油卡管理",menuList:[{name1:"油卡基本信息",msg:'basicInfo'},{name2:"油卡领用记录",msg:'collar'}
        ,{name3:"油卡充值记录",msg:'recharge'},{name4:"油卡使用记录",msg:'send'}], showIs: true},
