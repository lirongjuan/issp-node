...
app.controller('businessCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,costSer){
    $scope.navCla='Income';
    var active =$location.path().split('/')[3];
    $scope.navCla=active?active:'Income';
    $scope.navClass= function(name){
       $scope.navCla=name
    };
    costSer.navPermission().then(function(response){   //权限设置
    ...
    costSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"收入成本分析",menuList:[{name1:"收入成本分析",msg:'Income'}], showIs: true},
