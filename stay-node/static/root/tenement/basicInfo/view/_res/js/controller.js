...
app.controller('basicViewCtrl', function($scope,basicInfoSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    basicInfoSer.viewBasic(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.tenement.basicInfo.list[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.tenement.basicInfo.list[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/rental/downloadFile${encode(obj,true)}`;
