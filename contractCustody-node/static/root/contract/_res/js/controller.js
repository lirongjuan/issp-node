...
app.controller('businessCtrl', function ($scope,$state) {
    if ($state.current.url == '/contract') {//默认加载列表
        $state.go('root.contract.project');
    }
    //父 Ctrl 监听到事件，向下广播
    ...
    $scope.navCla=active?active:'project';
  ...
  contractSer.navPermission().then(function(response){   //权限设置
    ...
    $scope.showsList = [
        {id:"1",item:"合同保管管理",menuList:[{name1:"项目合同",msg:'project'},{name2:"房屋合同",msg:'house'},{name3:"租车协议",msg:'car'}],showIs:true},
