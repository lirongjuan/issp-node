...
app.controller('projectListCtrl',function($scope,projectSer,toastr,$stateParams,$state){
    ...
    function activatePage(page) {
        ...
        projectSer.listProject(pages).then(function(response){
    ...
    $scope.cancel = function(){ //取消删除
        $scope.delShow = false;
        $state.go('root.contract.project.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        projectSer.deleteProject(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.pagination.itemsCount-count)%10){
                    ...
                    $state.go('root.contract.project.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
