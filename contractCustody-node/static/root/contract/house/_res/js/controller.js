var app = angular.module('house', [{
    files:[
        "root/contract/house/_res/js/service.js"
    ]
}]);
app.controller('houseCtrl',function ($scope,$state) {

    if ($state.current.url == '/house') {//默认加载列表
    ...
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.contract.house.edit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.delete = function(){
        if($scope.getId){
            $state.go('root.contract.house.list[12]',{id:$scope.getId,name:"delete",page:$scope.page});
    ...
    $scope.upload = function(){
        if($scope.getId){
            $state.go('root.contract.house.upload[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.download = function(){
        if($scope.getId){
            $state.go('root.contract.house.view[12]',{id:$scope.getId,page:$scope.page});
