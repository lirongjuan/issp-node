var app = angular.module('houseView', ['toastr']);
app.controller('houseViewCtrl', function($scope,houseSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    houseSer.viewHouse(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.contract.house.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.contract.house.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/housingcontract/downloadFile${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloSigning.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/housingcontract/deleteFile',
                headers: {
