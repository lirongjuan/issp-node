var app = angular.module('business', [
    {files:[
    "root/finance/_res/js/service.js"
]}]);
app.controller('businessCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,financeSer){
   ...
   financeSer.navPermission().then(function(response){   //权限设置
    ...
    financeSer.setPermission().then(function(response){ //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"出车管理",menuList:[{name:'汇总',msg:'summary'},{name2:"用户用车",msg:'usecar'}],showIs:true},
