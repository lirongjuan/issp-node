var app = angular.module('statement', [
    {files:[
        "root/statement/_res/js/service.js"
    ]}
]);
app.controller('statementCtrl', function ($scope,$state) {
    ...
    $scope.$on('countId',function(event,msg){
        $scope.$broadcast('count',msg)
    });
}).controller('navCtrl',function($scope,$state,$location,statementSer){
    $scope.navCla='info';
   ...
   statementSer.navPermission().then(function(response){   //权限设置
    ...
    statementSer.setPermission().then(function(response){ //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList=[
        {id:"1",item:"银行流水管理",menuList:[{name:'银行账户信息'},{name2:"银行流水"},{name3:"汇总分析"}],showIs:true},
