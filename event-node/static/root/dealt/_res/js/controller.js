var app = angular.module('business', [
    {files:[
    "root/dealt/_res/js/service.js"
]}
]);
app.controller('businessCtrl', function ($scope,$state) {
    if ($state.current.url == '/dealt') {//默认加载列表
        $state.go('root.dealt.affair');
    }
    //父 Ctrl 监听到事件，向下广播
    $scope.$on("changeId",function(event,id){
        $scope.$broadcast("passId",id)
    });
    $scope.$on("changeName",function(event,name){
        $scope.$broadcast("passName",name)
    });
    $scope.$on("changePermissions",function(event,permissions){
        $scope.$broadcast("passPermissions",permissions)
    });
...
}).controller('navCtrl',function($scope,$state,$location,dealtSer){
    $scope.navCla='affair';
    var active =$location.path().split('/')[3];
    $scope.navCla=active?active:'affair';
    $scope.navClass= function(name){
       $scope.navCla=name
    };
    dealtSer.navPermission().then(function(response){   //权限设置
    ...
    dealtSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"待办事件",menuList:[{name1:"事件",msg:'affair'},{name2:"所有事件",msg:'allevent'},{name3:"提醒间隔时间设置",msg:'setTime'}], showIs: true},
