var app = angular.module('standard', [{
    files:[
        "root/emption/standard/_res/js/service.js"
    ]
}]);
app.controller('standardCtrl',function ($scope,$state) {
...
}).controller('standardMenuCtrl',function($scope,$state,$rootScope,$location,standardSer){
    ...
    $scope.menuCheck = function(name){
        ...
        standardSer.menuPermission(buttonName).then(function(response){
    ...
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.emption.standard.edit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.delete = function(){
        if($scope.getId){
            $state.go('root.emption.standard.list[12]',{id:$scope.getId,name:"delete",page:$scope.page});
