...
app.controller('manageViewCtrl', function($scope,manageSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    manageSer.viewManage(pathData).then(function(response){
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/contractmanage/downloadFile${encode(obj,true)}`;
