...
app.controller('settingCtrl',function ($scope,$state) {
    ...
    $scope.list = function(){
        $state.go('root.check.setting.list[12]');
        $scope.menuClass = 'listMenu'
    };
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.check.setting.edit[12]',{id:$scope.getId,page:$scope.page});
