...
app.controller('businessCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,checkSer){
    var active =$location.path().split('/')[3];
    $scope.navCla=active?active:'collar';
    $scope.navClass= function(name){
       $scope.navCla=name
    };
    checkSer.navPermission().then(function(response){   //权限设置
    ...
    checkSer.setPermission().then(function(response){   //权限设置
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"物资盘点",menuList:[{name1:"日盘",msg:'day'},{name2:"周盘",msg:'week'},{name3:"年盘",msg:'years'}], showIs: true},
        {id:"2",item:"物资分析",menuList:[{name4:'日盘',msg:'Aday'},{name5:'周盘',msg:'Aweek'},{name6:'年盘',msg:'Ayears'}], showIs: false},
