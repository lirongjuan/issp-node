var app = angular.module('week', [{
    files:[
        "root/check/week/_res/js/service.js"
    ]
}]);
app.controller('weekCtrl',function ($scope,$state) {
    ...
    $scope.edit = function(){
        if($scope.getId){
            $state.go('root.check.week.edit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.confirm = function(){
        if($scope.getId){
            $state.go('root.check.week.confirm[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.verify = function(){
        if($scope.getId){
            $state.go('root.check.week.verify[12]',{id:$scope.getId,page:$scope.page});
            $scope.menuClass='verifyMenu';
        }
    };
    $scope.aduit = function(){
        if($scope.getId){
            $state.go('root.check.week.aduit[12]',{id:$scope.getId,page:$scope.page});
    ...
    $scope.delete = function(){
        if($scope.getId){
            $state.go('root.check.week.list[12]',{id:$scope.getId,name:"delete",page:$scope.page});
