var app = angular.module('business', [
    {
        files: ['root/progress/_res/js/service.js']
    }
]);
app.controller('businessCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,progressSer){
    $scope.navCla='summary';
    var active =$location.path().split('/')[3];
    $scope.navCla=active?active:'summary';
    $scope.navClass= function(name){
        $scope.navCla=name;
    }
    //前面下拉导航权限
    progressSer.navPermission().then(function(response){
    ...
    //设置导航权限
    progressSer.setPermission().then(function(response){
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"资质进度管理",menuList:[{name:'资质进度汇总',msg:'summary'}],showIs:true},
